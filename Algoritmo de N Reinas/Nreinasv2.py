...
def inicializar_poblacion(num_pbl, j):
    global poblacion
    # Genera una poblacion de tamano j x num_pbl con valores aleatorios
    poblacion = [[random.randint(1, j) for _ in range(j)] + [0] for _ in range(num_pbl)]


# Evalua a un individuo basado en el num de reinas que no se atacan (a los lados y diagonalmente)
...
# Funcion de cruce, n puntos aleatorios
...
# Muta a un indivduo, basado en la probabilidad de mutacion
...
# Si encuentra un individuo con fitness = numero_reinas termina la busqueda
